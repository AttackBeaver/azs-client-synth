...
def generate_clients(n=1000):
    data = []
    for _ in range(n):
        client_type = random.choices(["Физическое лицо", "Юридическое лицо"], weights=[0.8, 0.2])[0]
        ...
        if client_type == "Физическое лицо":
            ...
            contract = "Нет"
            fuel_type = random.choices(["Бензин", "Дизель"], weights=[0.7, 0.3])[0]
            ...
            contract = "Да"
            fuel_type = random.choices(["Дизель", "Газ", "Бензин"], weights=[0.6, 0.3, 0.1])[0]
