...
def simulate_feature_response(clients_df: pd.DataFrame, 
    ...
    # найдем гипотезу
    feature = next((f for f in feature_hypotheses if f["feature_name"] == selected_feature), None)
    if feature is None:
        raise ValueError(f"Feature {selected_feature} not found in hypotheses.")
    ...
    for idx, row in df.iterrows():
        ...
        base_prob = base_rules.get(target_metric, 0.05)  # минимальная вероятность отклика
        ...
        prob = min(prob, 1.0)
        response = np.random.binomial(1, prob) if portrait in applicable_portraits else 0
